<?php
class Award_types extends Trongate {
    ...
    function create(): void {
        ...
        if ($update_id>0) {
            $data['headline'] = 'Update Award Type Record';
            $data['cancel_url'] = BASE_URL.'award_types/show/'.$update_id;
        } else {
            $data['headline'] = 'Create New Award Type Record';
            $data['cancel_url'] = BASE_URL.'award_types/manage';
        }

        $data['form_location'] = BASE_URL.'award_types/submit/'.$update_id;
    ...
    function manage(): void {
        ...
        if (segment(4) !== '') {
            ...
            $params['type'] = '%'.$searchphrase.'%';
            $sql = 'select * from award_types
            WHERE type LIKE :type
            ORDER BY id';
            $all_rows = $this->model->query_bind($sql, $params, 'object');
        } else {
            $data['headline'] = 'Manage Award Types';
        ...
        $pagination_data['pagination_root'] = 'award_types/manage';
        $pagination_data['record_name_plural'] = 'award types';
    ...
    function show(): void {
        ...
        if ($data == false) {
            ...
            $data['headline'] = 'Award Type Information';
            $data['view_file'] = 'show';
            $this->template('admin', $data);
        }
    }
    ...
    function submit(): void {
        ...
        if ($submit == 'Submit') {
            ...
            $this->validation_helper->set_rules('type', 'Type', 'required|min_length[2]|max_length[255]');
            ...
            if ($result == true) {
                ...
                if ($update_id>0) {
                    ...
                    $this->model->update($update_id, $data, 'award_types');
                    $flash_msg = 'The record was successfully updated';
                } else {
                    //insert the new record
                    $update_id = $this->model->insert($data, 'award_types');
                    $flash_msg = 'The record was successfully created';
                }

                set_flashdata($flash_msg);
                redirect('award_types/show/'.$update_id);
    ...
    function submit_delete(): void {
        ...
        if (($submit == 'Yes - Delete Now') && ($params['update_id']>0)) {
            ...
            $params['module'] = 'award_types';
            $this->model->query_bind($sql, $params);

            //delete the record
            $this->model->delete($params['update_id'], 'award_types');
    ...
    function _get_data_from_db(int $update_id): ?array {
        $record_obj = $this->model->get_where($update_id, 'award_types');
