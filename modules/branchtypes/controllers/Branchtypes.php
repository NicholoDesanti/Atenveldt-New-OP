<?php
class Branchtypes extends Trongate {
    ...
    function create(): void {
        ...
        if ($update_id>0) {
            $data['headline'] = 'Update Branchtype Record';
            $data['cancel_url'] = BASE_URL.'branchtypes/show/'.$update_id;
        } else {
            $data['headline'] = 'Create New Branchtype Record';
            $data['cancel_url'] = BASE_URL.'branchtypes/manage';
        }

        $data['form_location'] = BASE_URL.'branchtypes/submit/'.$update_id;
    ...
    function manage(): void {
        ...
        if (segment(4) !== '') {
            ...
            $sql = 'select * from branchtypes
            WHERE name LIKE :name
            ORDER BY id';
            $all_rows = $this->model->query_bind($sql, $params, 'object');
        } else {
            $data['headline'] = 'Manage Branchtypes';
        ...
        $pagination_data['pagination_root'] = 'branchtypes/manage';
        $pagination_data['record_name_plural'] = 'branchtypes';
    ...
    function show(): void {
        ...
        if ($data == false) {
            ...
            $data['headline'] = 'Branchtype Information';
            $data['view_file'] = 'show';
            $this->template('admin', $data);
        }
    }
        ...
        if ($submit == 'Submit') {
            ...
            if ($result == true) {
                ...
                if ($update_id>0) {
                    ...
                    $this->model->update($update_id, $data, 'branchtypes');
                    $flash_msg = 'The record was successfully updated';
                } else {
                    //insert the new record
                    $update_id = $this->model->insert($data, 'branchtypes');
                    $flash_msg = 'The record was successfully created';
                }

                set_flashdata($flash_msg);
                redirect('branchtypes/show/'.$update_id);
    ...
    function submit_delete(): void {
        ...
        if (($submit == 'Yes - Delete Now') && ($params['update_id']>0)) {
            ...
            $params['module'] = 'branchtypes';
            $this->model->query_bind($sql, $params);

            //delete the record
            $this->model->delete($params['update_id'], 'branchtypes');
    ...
    function _get_data_from_db(int $update_id): ?array {
        $record_obj = $this->model->get_where($update_id, 'branchtypes');
