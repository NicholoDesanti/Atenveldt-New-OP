<?php
class Honorary_titles extends Trongate {
    ...
    function create(): void {
        ...
        if ($update_id>0) {
            $data['headline'] = 'Update Honorary Title Record';
            $data['cancel_url'] = BASE_URL.'honorary_titles/show/'.$update_id;
        } else {
            $data['headline'] = 'Create New Honorary Title Record';
            $data['cancel_url'] = BASE_URL.'honorary_titles/manage';
        }

        $data['form_location'] = BASE_URL.'honorary_titles/submit/'.$update_id;
    ...
    function manage(): void {
        ...
        if (segment(4) !== '') {
            ...
            $sql = 'select * from honorary_titles
        ...
        foreach ($all_rows as $row) {
            ...
            $data['headline'] = 'Manage Honorary Titles';
        ...
        $pagination_data['pagination_root'] = 'honorary_titles/manage';
        $pagination_data['record_name_plural'] = 'honorary titles';
        ...
        $data['view_module'] = 'honorary_titles';
    ...
    function show(): void {
        ...
        if ($update_id == 0) {
            redirect('honorary_titles/manage');
        }
        ...
        if ($data == false) {
            ...
            $data['headline'] = 'Honorary Title Information';
            $data['view_file'] = 'show';
            $this->template('bootstrappy', $data);
        }
    }
        ...
        if ($submit == 'Submit') {
            ...
            if ($result == true) {
                ...
                if ($update_id>0) {
                    ...
                    $this->model->update($update_id, $data, 'honorary_titles');
                    $flash_msg = 'The record was successfully updated';
                } else {
                    //insert the new record
                    $update_id = $this->model->insert($data, 'honorary_titles');
                    $flash_msg = 'The record was successfully created';
                }

                set_flashdata($flash_msg);
                redirect('honorary_titles/show/'.$update_id);
    ...
    function submit_delete(): void {
        ...
        if (($submit == 'Yes - Delete Now') && ($params['update_id']>0)) {
            ...
            $params['module'] = 'honorary_titles';
            $this->model->query_bind($sql, $params);

            //delete the record
            $this->model->delete($params['update_id'], 'honorary_titles');
    ...
    function _get_data_from_db(int $update_id): ?array {
        $record_obj = $this->model->get_where($update_id, 'honorary_titles');
    ...
    function _get_branches_options($selected_key) {
        ...
        $options = $this->module_relations->_fetch_options($selected_key, 'honorary_titles', 'branches');
        return $options;
    }
      /**
